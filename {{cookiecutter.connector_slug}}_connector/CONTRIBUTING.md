...
1. Fork the `{{ cookiecutter.project_slug }}` repo on [GitHub](https://github.com/{{ cookiecutter.github_author }}/{{ cookiecutter.project_slug }}).

2. Clone your fork locally:

   ```bash
   git clone git@github.com:{your_username_here}/{{ cookiecutter.project_slug }}.git
   ```
...
7. Submit a pull request through the [GitHub](https://github.com/{{ cookiecutter.github_author }}/{{ cookiecutter.project_slug }}/pulls) website.
...
```bash
reuse annotate --copyright "{{ cookiecutter.author }}" --license "MIT" --recursive . --skip-unrecognised
```
...
> [!IMPORTANT]
> The message of the last commit must match the configured pattern, e.g. "Bump {{ cookiecutter.project_slug }} version: 0.1.0 → 0.1.1", for the publish job to run.
